...
pub fn register(vm: *VM) !void {
    ...
    const chardev_sym = try vm.intern("chardev?");
    try file_singleton.module.methods.put(chardev_sym, value.MethodEntry.builtin(&builtinFileChardev, .{ .exact = 1 }));
    ...
    const setgid_singleton_sym = try vm.intern("setgid?");
    try file_singleton.module.methods.put(setgid_singleton_sym, value.MethodEntry.builtin(&builtinFileSetgid, .{ .exact = 1 }));

    const setuid_singleton_sym = try vm.intern("setuid?");
    try file_singleton.module.methods.put(setuid_singleton_sym, value.MethodEntry.builtin(&builtinFileSetuid, .{ .exact = 1 }));
    ...
    const world_readable_singleton_sym = try vm.intern("world_readable?");
    try file_singleton.module.methods.put(world_readable_singleton_sym, value.MethodEntry.builtin(&builtinFileWorldReadable, .{ .exact = 1 }));
    ...
    const readable_real_sym = try vm.intern("readable_real?");
    try file_singleton.module.methods.put(readable_real_sym, value.MethodEntry.builtin(&builtinFileReadableReal, .{ .exact = 1 }));
    ...
    try filetest_singleton.module.methods.put(blockdev_sym, value.MethodEntry.builtin(&builtinFileBlockdev, .{ .exact = 1 }));
    try filetest_singleton.module.methods.put(chardev_sym, value.MethodEntry.builtin(&builtinFileChardev, .{ .exact = 1 }));
    try filetest_singleton.module.methods.put(pipe_sym, value.MethodEntry.builtin(&builtinFilePipe, .{ .exact = 1 }));
    ...
    try filetest_singleton.module.methods.put(owned_singleton_sym, value.MethodEntry.builtin(&builtinFileOwned, .{ .exact = 1 }));
    try filetest_singleton.module.methods.put(identical_sym, value.MethodEntry.builtin(&builtinFileIdentical, .{ .exact = 2 }));
    try filetest_singleton.module.methods.put(sticky_singleton_sym, value.MethodEntry.builtin(&builtinFileSticky, .{ .exact = 1 }));
    ...
    try filetest_singleton.module.methods.put(world_readable_singleton_sym, value.MethodEntry.builtin(&builtinFileWorldReadable, .{ .exact = 1 }));
    try filetest_singleton.module.methods.put(world_writable_singleton_sym, value.MethodEntry.builtin(&builtinFileWorldWritable, .{ .exact = 1 }));
    ...
    const blockdev_q_sym = try vm.intern("blockdev?");
    try vm.file_stat_class.module.methods.put(blockdev_q_sym, value.MethodEntry.builtin(&builtinFileStatBlockdevQ, .{ .exact = 0 }));
    ...
    const setgid_q_sym = try vm.intern("setgid?");
    try vm.file_stat_class.module.methods.put(setgid_q_sym, value.MethodEntry.builtin(&builtinFileStatSetgidQ, .{ .exact = 0 }));
        ...
        switch (std.c.errno(std.c.statx(std.os.linux.AT.FDCWD, path_z.ptr, flags, linux_statx_request, &statx))) {
            .SUCCESS => {
                return .{
                    ...
                    .rdev = gnuMakeDev(statx.rdev_major, statx.rdev_minor),
                };
            },
            .INTR => {
                try vm.checkAsyncEvents();
...
fn setFileStatIvars(vm: *VM, stat_val: Value, stat: std.Io.File.Stat, posix_metadata: PosixStatMetadata) VMError!void {
    ...
    try vm.setInstanceVariable(stat_val, "@rdev", Value.integer(posix_metadata.rdev));
...
pub fn builtinFileChardev(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    if (builtin.os.tag == .windows) {
        return vm.raiseExceptionFmt(vm.not_implemented_error_class, "File.chardev? is not implemented on Windows", .{});
    }

    const path = try vm.coerceToPath(args[0], "no implicit conversion into String");
    const st = std.Io.Dir.cwd().statFile(vm.io, path, .{}) catch return Value.boolean(false);
    return Value.boolean(st.kind == .character_device);
}

pub fn builtinFilePipe(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    if (builtin.os.tag == .windows) {
        return vm.raiseExceptionFmt(vm.not_implemented_error_class, "File.pipe? is not implemented on Windows", .{});
    }
...
pub fn builtinFileSetgid(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    if (builtin.os.tag == .windows) {
        return vm.raiseExceptionFmt(vm.not_implemented_error_class, "File.setgid? is not implemented on Windows", .{});
    }
    ...
    const stat = std.Io.Dir.cwd().statFile(vm.io, path_obj.str, .{}) catch return Value.boolean(false);
    const posix_metadata = try loadPosixStatMetadataForPath(vm, path_obj, @intCast(stat.permissions.toMode()), true);
    return Value.boolean((posix_metadata.mode & 0o2000) != 0);
}

pub fn builtinFileSetuid(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    if (builtin.os.tag == .windows) {
        return vm.raiseExceptionFmt(vm.not_implemented_error_class, "File.setuid? is not implemented on Windows", .{});
    }
    ...
    const stat = std.Io.Dir.cwd().statFile(vm.io, path_obj.str, .{}) catch return Value.boolean(false);
    const posix_metadata = try loadPosixStatMetadataForPath(vm, path_obj, @intCast(stat.permissions.toMode()), true);
    return Value.boolean((posix_metadata.mode & 0o4000) != 0);
}

pub fn builtinFileOwned(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    if (builtin.os.tag == .windows) {
        return vm.raiseExceptionFmt(vm.not_implemented_error_class, "File.owned? is not implemented on Windows", .{});
    }
...
pub fn builtinFileWorldWritable(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    ...
    const path_value = try vm.coerceToPathValue(args[0], "no implicit conversion into String");
    const path_obj = path_value.toStringObject();
    const stat = std.Io.Dir.cwd().statFile(vm.io, path_obj.str, .{}) catch return Value.nil();
    const posix_metadata = try loadPosixStatMetadataForPath(vm, path_obj, @intCast(stat.permissions.toMode()), true);
    const mode = posix_metadata.mode;
    if ((mode & 0o002) != 0) return Value.integer(mode & 0o7777);
    return Value.nil();
}

pub fn builtinFileWorldReadable(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    const path_value = try vm.coerceToPathValue(args[0], "no implicit conversion into String");
    const path_obj = path_value.toStringObject();
    const stat = std.Io.Dir.cwd().statFile(vm.io, path_obj.str, .{}) catch return Value.nil();
    const posix_metadata = try loadPosixStatMetadataForPath(vm, path_obj, @intCast(stat.permissions.toMode()), true);
    const mode = posix_metadata.mode;
    if ((mode & 0o004) != 0) return Value.integer(mode & 0o7777);
    return Value.nil();
}
...
pub fn builtinFileReadableReal(vm: *VM, _: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    if (builtin.os.tag == .windows) {
        return vm.raiseExceptionFmt(vm.not_implemented_error_class, "File.readable_real? is not implemented on Windows", .{});
    }
...
pub fn builtinFileStatBlockdevQ(vm: *VM, receiver: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 0);
    return fileStatBoolIvar(vm, receiver, "@blockdev");
}
...
pub fn builtinFileStatRdev(vm: *VM, receiver: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 0);
    return fileStatIntegerIvar(vm, receiver, "@rdev");
}

pub fn builtinFileStatRdevMajor(vm: *VM, receiver: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 0);
...
// setgid? returns true if the set-group-ID bit is set.
pub fn builtinFileStatSetgidQ(vm: *VM, receiver: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 0);
    const mode = try fileStatIntegerIvar(vm, receiver, "@mode");
    return Value.boolean(mode.isInteger() and (mode.toInteger() & 0o2000) != 0);
}
