...
pub fn register(vm: *VM) !void {
    ...
    const repeated_combination_sym = try vm.intern("repeated_combination");
    try vm.array_class.module.methods.put(repeated_combination_sym, value.MethodEntry.builtin(&builtinArrayRepeatedCombination, .{ .exact = 1 }));
...
pub fn builtinArrayDrop(vm: *VM, receiver: Value, args: []Value, _: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    const count = try args[0].coerceToI64ViaToInt(
        vm,
        "no implicit conversion into Integer",
    ...
    const array = receiver.toArrayObject();
    const out = try vm.createArray();
    const start: usize = @intCast(@min(count, @as(i64, @intCast(array.elements.items.len))));
    for (array.elements.items[start..]) |elem| {
        out.elements.append(vm.gc_allocator, elem) catch return error.Fatal;
    }
    return Value.fromObject(&out.object);
...
pub fn builtinArrayDropWhile(vm: *VM, receiver: Value, args: []Value, block: ?Block) VMError!Value {
    ...
    const blk = block orelse {
        const size_value = Value.integer(@intCast(receiver.toArrayObject().elements.items.len));
        return try vm.createMethodEnumeratorWithSize(receiver, try vm.intern("drop_while"), &.{}, size_value);
    };
    const source = receiver.toArrayObject();
    const result = try vm.createArray();
...
fn repeatedCombinationSize(len: i64, n: i64) i64 {
    if (n < 0) return 0;
    if (n == 0) return 1;
    if (len == 0) return 0;
    return combinationBinomialSize(len + n - 1, n);
}

pub fn builtinArrayRepeatedCombination(vm: *VM, receiver: Value, args: []Value, block: ?Block) VMError!Value {
    try vm.requireArgCount(args, 1);
    ...
    const blk = block orelse {
        const size = repeatedCombinationSize(len, n);
        return try vm.createMethodEnumeratorWithSize(
            receiver,
            try vm.intern("repeated_combination"),
            args,
            Value.integer(size),
        );
    };
    ...
    while (true) {
        const comb = try vm.createArray();
        for (indices) |idx| {
            comb.elements.append(vm.gc_allocator, elements[idx]) catch return error.Fatal;
        }
        _ = try vm.yieldToBlock(blk, &[_]Value{Value.fromObject(&comb.object)});
