...
pub const Big5Encoding = struct {
    ...
    pub fn nextCodepoint(_: Big5Encoding, bytes: []const u8, index: *usize) encoding.CodepointResult {
        ...
        index.* += 1;
        return .{ .valid = (second >= 0x40 and second <= 0x7e) or (second >= 0xa1 and second <= 0xfe), .len = 2, .codepoint = 0 };
    }

    pub fn nextChar(self: Big5Encoding, bytes: []const u8, index: *usize) encoding.CharResult {
        const result = self.nextCodepoint(bytes, index);
        return .{ .valid = result.valid, .len = result.len };
    }

    pub fn isValid(self: Big5Encoding, bytes: []const u8) bool {
        var index: usize = 0;
        while (index < bytes.len) if (!self.nextCodepoint(bytes, &index).valid) return false;
        return true;
    }

    pub fn isAsciiCompatible(_: Big5Encoding) bool { return true; }
    pub fn isDummy(_: Big5Encoding) bool { return false; }
