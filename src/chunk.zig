...
pub const Chunk = struct {
    ...
    fn disassembleInstruction(self: *Chunk, offset: usize, writer: *std.Io.Writer) !usize {
        ...
        switch (op) {
            .PUSH_NIL, .PUSH_TRUE, .PUSH_FALSE, .PUSH_SELF, .POP, .DUP, .SWAP, .CASE_MATCH, .OPT_PLUS, .OPT_MINUS, .OPT_MULT, .OPT_DIV, .OPT_EQ, .OPT_LT, .OPT_GT, .OPT_LE, .OPT_GE, .HALT, .TRY_END, .CATCH_END, .ENSURE_START, .ENSURE_END, .BREAK, .NEXT, .MULTI_ASSIGN_PREPARE, .ARRAY_APPEND, .ARRAY_CONCAT_ARRAY, .HASH_MERGE_KW, .YIELD_SPLAT, .BLOCK_GIVEN => {
