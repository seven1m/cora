  ...
  context "for hexadecimal literals" do
    ...
    ruby_version_is "3.4" do
      it "accepts a fractional part" do
        CORAFIXME "hex float fractional parts not supported yet", exception: ArgumentError do
    ...
    describe "with binary exponent" do
      %w(p P).each do |p|
        it "interprets the fractional part (on the left side of '#{p}') in hexadecimal" do
          CORAFIXME "hex float binary exponent (p/P) not supported yet", exception: ArgumentError do
